...
def hb_zone(spectral_type, app_mag, sy_dist, teff):
  ...
  if spectral_type in list(BC_constants.keys()):
    pass
  else:
    spectral_type = estimate_star_spect(teff)
...
for index, row in df.iterrows():
    if (row['f_a'] is not None) and ('None' not in str(row['f_a']))  and ('N/A' not in str(row['f_a'])) and (row['f_spec'] is not None) and ('None' not in str(row['f_spec']))  and ('N/A' not in str(row['f_spec'])) and (row['f_vmag'] is not None) and ('None' not in str(row['f_vmag']))  and ('N/A' not in str(row['f_vmag'])) and (row['f_dist'] is not None) and ('None' not in str(row['f_dist']))  and ('N/A' not in str(row['f_dist'])) and (row['f_teff'] is not None) and ('None' not in str(row['f_teff']))  and ('N/A' not in str(row['f_teff'])):
      try:
        name = row['f_preferred_name']
        orbsmax = float(row['f_a'])  #semi-major-axis of planet in AU
        spectral_type = str(row['f_spec'])[:2]
        if spectral_type == 'na':
          spectral_type = estimate_star_spect(row['f_teff'])
        app_Vmag = row['f_vmag']
        sy_dist = row['f_dist']
        inner_boundary, outer_boundary = hb_zone(spectral_type, app_Vmag, sy_dist, row['f_teff'])  #boudnaries fo habtiable zone
        if inner_boundary != 'NA':
          distance_from_star = (orbsmax - inner_boundary) / (outer_boundary - inner_boundary)  # normalized distance from star
          print(distance_from_star)
...
while run:
  ...
  try:
    ...
    for i in range(0, len(predicted_compositions)):
      print(f"{elements[i]}: {predicted_compositions[elements[i]]}")
  except KeyError:
    print("Invalid Input")
