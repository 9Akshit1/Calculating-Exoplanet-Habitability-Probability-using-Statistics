function makeRanks() {
  fetch('/data_folder_5/star_total_RHP.csv') 
    ...
    .then(csvData => {
        ...
        for (let i = 1; i < rows.length; i++) {
          vals = rows[i].split(',')
          names_RHP.push([vals[0], String(parseFloat(vals[1]).toFixed(2)) + ' AV.RHP'])
        }
...
function populateLeaderboard() {
  ...
  .then(csvData => {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        let name_RHP = [vals[0], String(parseFloat(vals[1]).toFixed(2)) + ' AVG.RHP'];
